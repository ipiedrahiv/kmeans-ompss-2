...
void kMeansClustering(double* points, int N, int k, size_t maxIter, double tol, int numFeatures) {
    ...
    int* label = (int*) malloc(sizeof(double)*N);
    ...
    for (size_t iter = 0; iter < maxIter; iter++) {
        ...
        for (int point = 0; point < N; point++) {
            double* currentPoint = &points[point*numFeatures];
        ...
        for (int centroid = 0; centroid < k; centroid++) {
            double* oldCentroid = &centroids[centroid*numFeatures];
            for (int feature = 0; feature < numFeatures; feature++) {
                centroids[centroid*numFeatures+feature] = sum[centroid*numFeatures+feature]/nPoints[centroid];
            }
            double* newCentroid = &centroids[centroid*numFeatures];
